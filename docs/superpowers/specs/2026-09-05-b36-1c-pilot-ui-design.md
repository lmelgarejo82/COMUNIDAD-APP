...
Ruling: the existing admin `PUT /api/bookings/:id/status` body requires `{status, expected_status}`; the sole in-repo caller, `bookingService.updateStatus`, sends the selected record's observed state. Missing/malformed expected state returns400; a mismatch after the scoped transaction lock returns409 without a mutation. A lock without the observed state cannot distinguish a stale competing cancel from a deliberate later cancel. Tradeoff: external legacy callers that omit this field need the same minimal update; endpoint and role semantics are preserved.

The creation summary and selected-booking detail display dates with explicit24-hour time (`hourCycle: 'h23'`) in the existing local timezone. Actual runtime `es-AR` defaults rendered13:00 as01:00 without a meridiem marker; this bounded presentation correction leaves stored timestamps and scheduling policy unchanged.
