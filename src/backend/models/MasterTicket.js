...
async function validateAffectedUnit(db, communityId, entry) {
  let query;
  let id;
  if (entry.unit_id) {
    id = entry.unit_id;
  ...
  const { rows } = await db.query(query, [id, communityId]);
...
const MasterTicket = {
  async createMasterTicket({ community_id, title, description, type, file_url, created_by }, affectedUnits = [], client = null) {
    ...
    try {
      ...
      for (const entry of affectedUnits) {
        if (!(await validateAffectedUnit(db, community_id, entry))) {
          const error = new Error('MASTER_TICKET_SCOPE_INVALID');
          error.code = 'MASTER_TICKET_SCOPE_INVALID';
          throw error;
        }
      }
  ...
  async getMasterTicket(id, communityId) {
    const { rows: masterRows } = await pool.query(
      `SELECT mt.*, u.email AS created_by_email
       ...
       WHERE mt.id = $1 AND mt.community_id = $2`,
      [id, communityId]
    );
    ...
    const [affected, subTickets] = await Promise.all([
      pool.query(
        `SELECT mtu.*, u.unit_code, b.name AS building_name, f.number AS floor_number
         ...
         WHERE mtu.master_ticket_id = $1
           AND (
             (mtu.unit_id IS NOT NULL AND EXISTS (
               SELECT 1 FROM units scoped_unit
               JOIN floors scoped_floor ON scoped_unit.floor_id = scoped_floor.id
               JOIN buildings scoped_building ON scoped_floor.building_id = scoped_building.id
               JOIN complexes scoped_complex ON scoped_building.complex_id = scoped_complex.id
               WHERE scoped_unit.id = mtu.unit_id AND scoped_complex.community_id = $2
             ))
             OR (mtu.floor_id IS NOT NULL AND EXISTS (
               SELECT 1 FROM floors scoped_floor
               JOIN buildings scoped_building ON scoped_floor.building_id = scoped_building.id
               JOIN complexes scoped_complex ON scoped_building.complex_id = scoped_complex.id
               WHERE scoped_floor.id = mtu.floor_id AND scoped_complex.community_id = $2
             ))
             OR (mtu.building_id IS NOT NULL AND EXISTS (
               SELECT 1 FROM buildings scoped_building
               JOIN complexes scoped_complex ON scoped_building.complex_id = scoped_complex.id
               WHERE scoped_building.id = mtu.building_id AND scoped_complex.community_id = $2
             ))
           )`,
        [id, communityId]
      ),
      pool.query(
        `SELECT t.*, u.email AS user_email
         ...
         WHERE t.master_ticket_id = $1 AND t.community_id = $2
  ...
  async updateMasterTicket(id, communityId, { title, description, type, status, file_url }) {
    const fields = [];
    const params = [id, communityId];
    ...
    if (fields.length === 0) {
      const { rows } = await pool.query(
        'SELECT * FROM master_tickets WHERE id = $1 AND community_id = $2',
        [id, communityId]
      );
      return rows[0] || null;
    }
    ...
    const { rows } = await pool.query(
      `UPDATE master_tickets SET ${fields.join(', ')}
    ...
    try {
      ...
      const { rows: masterRows } = await client.query(
        `SELECT id FROM master_tickets
         ...
         FOR UPDATE`,
        [masterId, communityId]
      );
      if (!masterRows[0]) {
        await client.query('COMMIT');
        return { ticket: null, master_closed: false };
      }

      const { rows: ticketRows } = await client.query(
        `UPDATE tickets SET status = 'resolved', updated_at = NOW()
         WHERE id = $1
           AND master_ticket_id = $2
           AND community_id = $3
           AND status != 'resolved'
         RETURNING *`,
        [ticketId, masterId, communityId]
      );
      ...
      if (parseInt(countRows[0].remaining) === 0) {
        ...
        masterClosed = Boolean(masterRows[0]);
      }

      await client.query('COMMIT');
      return { ticket, master_closed: masterClosed };
    } catch (err) {
